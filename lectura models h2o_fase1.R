...
rf_fisica <- h2o.loadModel("C:/Users/Usuario/Documents/Tesis/datos/base de datos arreglada/test_grid_rf_model_88")
...
roc_rf_fisica <- data.frame(modelo=rep("random_forest",48),roc_rf_fisica)


# leer modelo rna para fisica ----

rna_fisica <- h2o.loadModel("C:\\Users\\Usuario\\Documents\\Tesis\\datos\\base de datos arreglada\\dl_grid_fisica_model_45")
...
roc_rna_fisica <- data.frame(modelo=rep("rna",51),roc_rna_fisica)

roc_curves<- rbind(roc_rf_fisica,roc_logistic,roc_rna_fisica,roc_svm)
# modelos para el resto de las carreras -----


# leer modelo rf para el resto ----
rf_fcuv <- h2o.loadModel("C:/Users/Usuario/Documents/Tesis/datos/base de datos arreglada/test_grid_rf_fcuv_model_328")
...
roc_rf_fcuv <- data.frame(modelo=rep("random_forest",154),roc_rf_fcuv)


# leer modelo rna para el resto -----

rna_fcuv <- h2o.loadModel("C:\\Users\\Usuario\\Documents\\Tesis\\datos\\base de datos arreglada\\dl_grid_fcuv_model_270")
...
ggplot(roc_curves,aes(fpr,tpr))+geom_line(aes(color=modelo),size=1.5)+ geom_abline(slope = 1)+
  ...
  ggtitle('Curva ROC para los modelos de la fase 1 para física')+
...
ggplot(roc_curves,aes(fpr,tpr))+geom_line(aes(color=modelo),size=1.5)+ geom_abline(slope = 1)+
  xlab('Tasa de Falsos positivos')+
  ylab('Tasa de verdaderos positivos')+
  ggtitle('Curva ROC para los modelos de la fase 1 para Cluster 2')+
  scale_color_discrete(name= "Modelo",
                       labels=c("Modelo logístico","Random forest",
                                "Red neuronal artificial","Máquina de vectores de soporte"))+
  theme(axis.text=element_text(size=12),
