...
variables %>% ggplot ( aes(x=promedio)) + geom_histogram(binwidth = 0.1)
# variables<-variables %>%
...
f <- formula ( retiro ~Genero +lugar+Puntaje_Seleccion+rama_educacional+Dependencia+
                 Gratuidad+ingreso1+cociente+promedio)
log_reg<-glm(f, data = train_fisica, family = 'binomial')
log_fisica<-summary(log_reg)
xtable(log_fisica)


pred1<- predict.glm(log_reg,newdata = test_fisica[-14], type="response")
library(ROCR)
pred = ROCR::prediction(pred1,test_fisica[14])
...
precision = c()
recall=c()
for(i in seq(from = 0.01 , to = 0.7 , by = 0.01)){
  k = k + 1
  preds_binomial = ifelse(pred1 > i , 1 , 0)
  confmat = table(test_fisica$retiro , preds_binomial)
...
threshold=seq(from = 0.01 , to = 0.7 , by = 0.01)
...
preds.for.50 = ifelse(pred1 > 0.24 , 1 , 0)
confusion = table(Predicted = preds.for.50 , Actual = test_fisica$retiro)
caret::confusionMatrix(confusion)



roc_logistic <- data.frame(modelo=rep("logistic",70),
                           tpr=data$recall,
...
# sin puntaje----
f3 <- formula( retiro ~ lugar+Puntaje_Seleccion+Gratuidad+demora_ingreso+promedio)
...
precision = c()
recall=c()
for(i in seq(from = 0.05 , to = 0.9 , by = 0.01)){
  k = k + 1
  preds_binomial = ifelse(pred1 > i , 1 , 0)
...
threshold=seq(from = 0.05 , to = 0.9 , by = 0.01)
data = data.frame(threshold , accuracy  ,recall,specifity)
ggplot(gather(data , key = 'Metric' , value = 'Value' , 2:3) , 
       aes(x = threshold , y = Value , color = Metric)) + 
  geom_line(size = 1.5)+labs(x= "Umbral",
